        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-gray-900">Редактировать ребенка</h1>
                ...
                <form method="POST" action="{{ route('admin.children.update', $child) }}" class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                                ...
                                @if($child->hasAvatar())
                                    <div class="mt-2">
                                        <label class="flex items-center">
                                            <input type="checkbox" name="remove_avatar" value="1" class="rounded border-gray-300 text-red-600 shadow-sm focus:border-red-500 focus:ring-red-500">
                                            <span class="ml-2 text-sm text-red-600">Удалить текущую фотографию</span>
                                        </label>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            Текущий возраст: <span class="font-medium">{{ $child->age_text }}</span>
                        </p>
                    </div>
                            ...
                            @foreach($squads as $squad)
                                <div class="flex items-center">
                                    <input id="squad_{{ $squad->id }}" 
                                           ...
                                           type="checkbox" 
                                           value="{{ $squad->id }}"
                                           {{ in_array($squad->id, old('squads', $child->squads->pluck('id')->toArray())) ? 'checked' : '' }}
                        ...
                        @if($child->squads->count() > 0)
                            <p class="mt-1 text-sm text-gray-500">
                                ...
                                @foreach($child->squads as $squad)
                                    <span class="font-medium">{{ $squad->name }}</span>@if(!$loop->last), @endif
                                @endforeach
                            </p>
                        @endif
                    ...
                    <div>
                        ...
                        <textarea name="notes" 
                                  ...
                                  placeholder="Любая дополнительная информация о ребенке...">{{ old('notes', $child->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-200">
                    <div class="bg-red-50 p-4 rounded-lg">
                        <h3 class="text-lg font-medium text-red-900 mb-2">Опасная зона</h3>
                        <p class="text-sm text-red-700 mb-4">
                            Удаление ребенка нельзя отменить. Все связанные данные будут потеряны.
                        </p>
                        
                        <form method="POST" 
                              action="{{ route('admin.children.destroy', $child) }}" 
                              onsubmit="return confirm('Вы уверены, что хотите удалить ребенка {{ $child->full_name }}? Это действие нельзя отменить.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
...
<script>
// Показываем новый возраст при изменении даты рождения
document.getElementById('birth_date').addEventListener('change', function() {
    ...
    if (age >= 0 && age <= 100) {
        // Обновляем текст с возрастом
        const ageText = age + (age % 10 == 1 && age % 100 != 11 ? ' год' : 
                              (age % 10 >= 2 && age % 10 <= 4 && (age % 100 < 10 || age % 100 >= 20) ? ' года' : ' лет'));
        
        const ageDisplay = document.querySelector('p.text-gray-500 span.font-medium');
        if (ageDisplay) {
